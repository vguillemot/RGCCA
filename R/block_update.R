...
block_update.graphnet_block <- function(x, grad, mu) {
  ...
  if (x$graph_laplacian$lambda==0) {
    x$a = 0.5*a_grad/mu + 0.5*block_project(x)$a
    x$Y <- pm(x$x, x$a, na.rm = x$na.rm)
    return(x)
  }
  if (is.null(x$graph_laplacian$mu) || mu != x$graph_laplacian$mu) {
    x$graph_laplacian$mu = mu
    #LDLTsolver_compute(x$graph_laplacian$solver@pointer, 2*mu*diag(length(x$a)) + x$graph_laplacian$L)
    x$graph_laplacian$Linv = compute(x$graph_laplacian, 2*mu*diag(length(x$a)) + 2*x$graph_laplacian$L)
  }
